...
class LoRAAdapter(nn.Module):
    ...
    def __init__(self, base: nn.Linear, rank: int = 8, scale: float = 1.0):
        ...
        # A: down-project, zero-init not needed (Kaiming is fine)
        self.lora_A = nn.Linear(in_features, rank, bias=False)
        # B: up-project, zero-init so LoRA starts as identity
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # LoRA adapters wrap c_q and c_v (most impactful for attention)
        self.lora_q = LoRAAdapter(self.c_q, rank=LORA_RANK, scale=LORA_SCALE)
        self.lora_v = LoRAAdapter(self.c_v, rank=LORA_RANK, scale=LORA_SCALE)

    def forward(self, x: Tensor) -> Tensor:
        bsz, seqlen, dim = x.shape
        q = self.lora_q(x).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
        v = self.lora_v(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self.proj._zero_init = True
        # LoRA adapter on the up-projection (fc)
        self.lora_fc = LoRAAdapter(self.fc, rank=LORA_RANK, scale=LORA_SCALE)

    def forward(self, x: Tensor) -> Tensor:
        x = torch.relu(self.lora_fc(x))
...
def main() -> None:
    ...
    matrix_params = [
        ...
        if p.ndim == 2 and p.requires_grad and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
        and 'lora_' not in name  # LoRA params get their own optimizer
    ]
    lora_params = [
        p
    ...
    # LoRA params use Adam with a moderate learning rate
    optimizer_lora = torch.optim.Adam(
        [{"params": lora_params, "lr": args.scalar_lr, "base_lr": args.scalar_lr}],
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        fused=True,
    ...
    if master_process:
        torch.save(ema_model.state_dict(), "final_model.pt")
    ...
    quant_obj, quant_stats = quantize_state_dict_int8(ema_model.state_dict())
    ...
    ema_model.load_state_dict(dequantize_state_dict_int8(quant_state), strict=True)
    torch.cuda.synchronize()
