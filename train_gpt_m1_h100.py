...
class UnigramProj(nn.Module):
    """Codec Layer 2: Project per-token unigram log-probability into model space."""
    def __init__(self, model_dim: int):
        super().__init__()
        self.proj = nn.Linear(1, model_dim, bias=True)
        self.scale = nn.Parameter(torch.ones(model_dim) * 0.1)
    
    def forward(self, input_ids: Tensor, unigram_logprobs: Tensor) -> Tensor:
        # Look up unigram log-prob for each token
        logprobs = unigram_logprobs[input_ids.long()].unsqueeze(-1)  # [bsz, seq, 1]
        return self.proj(logprobs) * self.scale[None, None, :]
...
class GPT(nn.Module):
    def __init__(
        ...
        self.unigram_proj = UnigramProj(model_dim)
